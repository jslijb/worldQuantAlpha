...
- **豁免路径实质已死**：豁免线 = 1.10×max(corr≥0.7 对手的 S)，而最高 S 对手是自己提交的 `0mR2K6lr`(S=3.45) → 线在 3.795，全池最强候选仅 3.05。**唯一出路是 corr < 0.70 直通。**
...
## corr 服务与探测（0915 定案）
- `200 + Retry-After + 空 body` = **平台正在现算，须继续轮询**，**不是故障**。低负载时轮询 3–4 次（4–6 秒）即返回 records；高负载时会挂到 120–180s。
- 此前连续 30+ 小时判"服务停摆"是 `probe_corr_service.py` 的 `max_tries=3`（≈6 秒就放弃）判据写错，**已改为时间预算制**。**提交链路一直可用**——代价是 47+ 条达标候选白积压。教训：**探针判据写错比服务故障更能拖垮进度。**
- 秒回（0.3s）的 id 基本是**命中缓存**；同一 id 先挂 180s 后又秒回 → 是**计算排队/限流**。
- **⛔ 行动规则：不要在提交前额外做 corr 探测。** 探测挤占**账号级 corr 计算配额**，正式提交时就拿不到结果；`submit_v2.corr_precheck` 自带 200 次轮询，**交给它，人工别插队**。
