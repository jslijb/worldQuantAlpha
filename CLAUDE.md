...
| **★ 拿 corr 数值的唯一确凿入口** | **只有 alpha 提交成功之后**，`GET /alphas/{id}` 的 `is.selfCorrelation` 才有值（实证 w122_a `akLp3pzW` = **0.8036**，status=ACTIVE/stage=OS）。⚠️ **反向修正**：`is.checks` 里的 `SELF_CORRELATION=PENDING` **不是"正在计算"的信号**，而是**所有未提交 alpha 的默认占位符**（对照 `O0r3epMg`/`le8mxdn7`/`3q9NMgk6`/`6XrkW6R5`/`KPOAOXEE` 5 条从未触碰的候选，全部 checks=8 条、全部 PENDING）—— 别用它判断提交是否触发。**未提交状态下除 `correlations/self` 外无取数入口**，而该端点当前取不到数。提交器：`src/submit/blind_submit.py`（盲提交，一次一个，**PENDING 即停手**）、`src/submit/submit_v2.py`（带 corr 预检，预检不可用时每条 150 秒 TIMEOUT 后跳过） |
