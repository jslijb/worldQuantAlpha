...
> **归档信息**
> - 页面：<https://platform.worldquantbrain.com/learn/documentation/alpha-submission>
> - 页面 id：`alpha-submission` ｜ 课程：`Getting Started` ｜ 预计时长：PT11M
> - 平台最后更新：2026-09-15T01:31:48.138796-04:00
...
![article_6_core_requirements.png](../images/alpha-submission/01_article_6_core_requirements.png)


| SUBMISSION CRITERIA | THRESHOLDS FOR USERS |
|---|---|
| Fitness | At least “Average”:
...
| Sub universe test | The Sharpe in the sub universe must be higher than at least one threshold. 
...
![check111.png](../images/alpha-submission/02_check111.png)


![Subuniverse 3.png](../images/alpha-submission/03_Subuniverse_3.png)
...
![subuniverse 5.png](../images/alpha-submission/04_subuniverse_5.png)


![subinverse 6.png](../images/alpha-submission/05_subinverse_6.png)

As you see, Sharpe ratio degraded significantly to 1.17, less than the cutoff of 1.18.


![article_6_subuniverse_test.png](../images/alpha-submission/06_article_6_subuniverse_test.png)
...
![article_6_special_alpha_types.png](../images/alpha-submission/07_article_6_special_alpha_types.png)


### ATOM Alphas

- ATOM Alphas are Alphas that use fields from only 1 dataset. The following grouping fields are excluded when counting datasets and do not disqualify an Alpha from being an ATOM: currency, country, exchange, sector, industry, subindustry, market, split, adjfactor
...
- Sharpe >= 1.0
- Number of total operators <= 8 (including repeat operators). The ts_backfill and group_backfill operators are not counted toward this limit.
- Number of unique data fields (excluding grouping fields) <= 3
- Grouping fields: country, industry, subindustry, currency, market, sector, exchange, split, adjfactor
- Power Pool Correlation < 0.5. If the Power Pool correlation is > 0.5, the Alpha must have a Sharpe at least 10% higher than the most correlated Alpha to be considered eligible for submission.
- Once you tag an Alpha as power pool, it stays in the self-correlation pool even if you untag it later
- The Alpha must pass turnover tests
- The Alpha must pass the sub-universe test
- The Alpha must pass the robust-universe test (where applicable)
- Eligible region, delay, universe and other conditions (risk neutralization, datasets, turnover ranges, etc) are defined by the active Power Pool theme (e.g. GLB/D1, All regions/D0, EUR/D1 Fast Factors)
- An Alpha description is required — it must include the Idea, the Rationale for data used, and the Rationale for operators used
...
![Picture1.png](../images/alpha-submission/08_Picture1.png)
...
![Picture1.png](../images/alpha-submission/09_Picture1.png)
