...
- ★ isScore 是"表现分"不是累计积分：官方 scoring=PERFORMANCE；实测 0925→0926 净 −177（8923→8746，rank 50→57），且 ΣS/ΣF 对不上总数 ⇒ 非逐条求和，是合并池表现动态重算、会掉。第三方"单条 1500-2000 分/日上限 2000"与实测日均 ~291 分矛盾，弃用。含义：低 corr 互补候选边际贡献 > 高 selfCorr 条；OS 差的条目拖累合并分。锚点序列 `_autologs/_isscore_history.csv`（自动化每日追加），攒 2 周回归定死。
