...
- **本标准锁死**：任何修改（放宽或收紧）必须先获李工审批；自动化与会话执行一律不得自行放宽或另立口子。
- 每日任务书等指令中与本节冲突的质量判据（含 0922 修订的"tS≥1.0、TO≤50% 可直接交"）**自 0926 起作废**，一律以本节为准。
- 0925~0926 曾按放宽口子交过贴线货（TO 44~46%、tS 1.08、selfCorr 0.69），合并表现分净掉 177（8923→8746）——这是本节恢复锁死的原因，留档防再犯。

### ★ CLAUDE.md 修改纪律（李工 0926 定）

- **修改 CLAUDE.md 任何内容必须先获李工审批**，获批后才能动文件。
- 李工要求加入 CLAUDE.md 的内容必须执行。
...
| 日期 | 变更 |
|---|---|
| 2026-09-26 | **李工定死提交质量标准**（S+F≥4.0 / tS≥1.25 / TO≤20% / 无 FAIL，修改必须审批）；0922 放宽口子"tS≥1.0、TO≤50%"作废；新增 CLAUDE.md 修改纪律（改本文件必须李工审批）。起因：0925~0926 未经显式审批放宽标准交贴线货，合并表现分掉 177 |
