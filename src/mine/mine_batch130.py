...
DPQ='fnd6_newqv1300_dpactq/assets'; ANO='fnd6_newa1v1300_ano/assets'; MIB='fnd6_mfmq_mibtq/assets'
...
C = [
 # a: 最强锚 1.5 权重（w129_b 底子）
 ('w130_a', f'1.5*{G(TXA)} + {G(AOL)} + {G(TXP)} + 0.5*{PVD5} + 0.5*{VOL12}', BASE()),
 # b: 去重税系，换 D&A 锚
 ('w130_b', f'1.5*{G(TXA)} + {G(AOL)} + {G(DPQ)} + 0.5*{PVD5} + 0.5*{VOL12}', BASE()),
 # c/d: decay 0 / 8
 ('w130_c', f'{G(TXA)} + {G(AOL)} + {G(TXP)} + 0.5*{PVD5} + 0.5*{VOL12}', BASE(delay=0)),
 ('w130_d', f'{G(TXA)} + {G(AOL)} + {G(TXP)} + 0.5*{PVD5} + 0.5*{VOL12}', BASE(delay=8)),
 # e: VOLB 桶
 ('w130_e', ' + '.join([GB(TXA,VOLB), GB(AOL,VOLB), GB(TXP,VOLB), f'0.5*{PVD5}', f'0.5*{VOL12}']), BASE()),
 # f: 四锚 1.5/1/1/1（全部去重）
 ('w130_f', f'1.5*{G(TXA)} + {G(AOL)} + {G(DPQ)} + {G(MIB)} + 0.5*{PVD5} + 0.5*{VOL12}', BASE()),
 # g: truncation 0.05
 ('w130_g', f'{G(TXA)} + {G(AOL)} + {G(TXP)} + 0.5*{PVD5} + 0.5*{VOL12}', BASE(trunc=0.05)),
 # h: PV 腿加到 0.75（验证期稳定性换挡）
 ('w130_h', f'{G(TXA)} + {G(AOL)} + {G(TXP)} + 0.75*{PVD5} + 0.75*{VOL12}', BASE()),
]
