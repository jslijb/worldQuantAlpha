...
for _d in [_p.parent, *_p.parents]:
    if (_d / 'brain_credentials.txt').exists():
        _os.chdir(_d); break
# ============================================================
# AlphaSimulator.py — 生产级 Alpha 并发模拟管理器
# 功能：
#   - 从 CSV 文件中分批读取待模拟的 Alpha 列表
#   - 支持最大并发数控制，循环提交与轮询模拟状态
#   - 自动处理登录过期、断线重连、失败重试（含容忍上限）
#   - 将模拟完成的 Alpha 结果写入日期命名的 CSV
#   - 将失败的 Alpha 记录至 fail_alphas.csv
# 用法：python AlphaSimulator.py（需提前准备好 alpha_list_pending_simulated.csv）
# ============================================================
import requests
import json
